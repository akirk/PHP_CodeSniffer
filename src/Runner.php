...
class Runner
{
     ...
     *
     * @param array $realDiff The real diff data generated by test-applying the fix.
     *
     * @return string The formatted diff output.
     */
    private function generateDiffDisplay(array $realDiff)
    {
        $output = '';

        foreach ($realDiff as $diffLine) {
    ...
    private function handleSingleViolation(array $message, int $line, int $column, File $file)
    {
        ...
        if ($hasInteractiveFixes) {
            ...
            foreach ($interactiveFixOptions as $index => $fixOption) {
                ...
                if (isset($fixOption['realDiff'])) {
                    echo $this->generateDiffDisplay($fixOption['realDiff']);
                }
            }
            echo PHP_EOL;
        }
